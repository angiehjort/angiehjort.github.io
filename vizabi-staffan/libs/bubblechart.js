...
var BubbleChart = Vizabi.Component.extend({
  ...
  updateMarkerSizeLimits: function updateMarkerSizeLimits() {
    ...
    if (this.model.marker.size.scaleType !== "ordinal") {
      ...
      this.sScale.range(this.sScale.domain().length === 1 ? [(minArea + maxArea) / 2] : d3.range(minArea, maxArea, (maxArea - minArea) / this.sScale.domain().length));
    }
  },
