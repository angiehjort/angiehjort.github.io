...
Model = function(dataTree, dataRules, dataNoActions){
    var _this = this;

    this.root = dataTree;
    this._root = JSON.parse(JSON.stringify(dataTree));
    this.rootHeb = JSON.parse(JSON.stringify(dataTree));
    this.dataRules = dataRules;
    
    this.noActions = dataNoActions.Results.alarmsWithNoAction;
    ...
    this.generateLinks = function(possibleNodes, howMany){
        ...
        for(var i=0; i<howMany; i++){
            links.push({
                source: possibleNodes[Math.floor(Math.random()*possibleNodes.length)],
                target: possibleNodes[Math.floor(Math.random()*possibleNodes.length)],
    ...
    this.getNodeByNodeID = function(id, within){
        if(within == null) within = _this.root;
        return _this.getAllNodes(within).filter(function(d){return d.id==id})[0];
    };
    ...
    this.getLinksByNode = function(node, inSources, inTargets, includeNestedNodes){
        //defaults
        if (inSources==null)inSources=true;
        if (inTargets==null)inTargets=true; //by default
        ...
        if (includeNestedNodes){
            ...
            _this.getLeavesEvenHidden(node).forEach(function(n){
                result = result.concat(
                    _this.links.filter(function(d){return inSources && d.source==n.id || inTargets && d.target==n.id})
                );
            });
            return arrayUnique(result);
        }else{
            return _this.links.filter(function(d){return inSources && d.source==node.id || inTargets && d.target==node.id});
        }
    };
